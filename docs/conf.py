...
# built documents.
#
release = version = niwidgets.__version__
...
man_pages = [(master_doc, "niwidgets", "niwidgets Documentation", [author], 1)]
...
intersphinx_mapping = {
    ...
    "np": ("https://docs.scipy.org/doc/numpy-1.10.0/", None),
}
    ...
    def run(self):
        ...
        try:
            ...
            if "methods" in self.options:
                ...
                _, supmethods = self.get_members(
                    super(c), "method", ["__init__"]
                )

                self.content = [
                    "~%s.%s" % (clazz, method)
                    for method in methods
                    if not method.startswith("_") and method in c.__dict__
                ]
